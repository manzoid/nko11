...
Rect.prototype.collidePoint = function() {
   var args = normalizeRectArguments.apply(this, arguments);
   //console.log(args.left + ' - ' + args.top + ' ///// ' + this.left + ' - ' + this.right + ' - ' + this.top + ' - ' + this.botto);
 ...
 * @name preload
 */
var preload = exports.preload = function(resources, postload) {
   var resourceMap = RESOURCES;
   if (postload) {
     // Note this still works because the global that load()
     // checks is CACHE, not RESOURCES.
     resourceMap = {};
   }
   ...
   resources.forEach(function(res) {
      // normalize slashses
      resourceMap[res] = resourceBaseHref() + ('/' + res).replace(/\/+/g, '/');
   }, this);
   //alert(RESOURCES.toSource())
...
exports.init = function() {
   ...
   // Hack in minimal touchscreen support so we can react to iPad clicks...
   $('#gjs-canvas')[0].addEventListener('touchstart', function(e){
     ...
     // This stuff might be necessary if we don't position the canvas
     // right up at 0,0... as it stands we get a free pass because there
     // is no actual canvas offset to deal with.
     //var canvasOffset = display._getCanvasOffset();
     //alert('xxx ' + canvasOffset[0] + ' ----- ' + canvasOffset[1]);
     var fakeEvent = {
       'clientX': touch.pageX,
       'clientY': touch.pageY,
       'button': e.button,
 ...
 */
exports.load = function(key, opt_width, opt_height) {
   //window.console.log('Trying to load: ' + key);
   var img;
   ...
   canvas.width = opt_width || img.naturalWidth || img.width;
   canvas.height = opt_height || img.naturalHeight || img.height;
   var context = canvas.getContext('2d');
   if (opt_width && opt_height) {
     context.drawImage(img, 0, 0, opt_width, opt_height);
   } else {
     context.drawImage(img, 0, 0);
   }
   img.getSize = function() { return [opt_width || img.naturalWidth, opt_height || img.naturalHeight]; };
